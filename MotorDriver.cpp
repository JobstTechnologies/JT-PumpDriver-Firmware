...
void MotorDriver::setMotor(uint8_t motor, int16_t val, bool useBrakes){
  if(useBrakes){
    if(motor==1){
      if(val>0){
        writeCommand(COMMAND_MOTOR_1,_PWMperiodInt-abs(val),_PWMperiodInt);
      }else{
        writeCommand(COMMAND_MOTOR_1,_PWMperiodInt,_PWMperiodInt-abs(val));
      }
    }else if(motor==2){
      if(val>0){
        writeCommand(COMMAND_MOTOR_2,_PWMperiodInt-abs(val),_PWMperiodInt);
      }else{
        writeCommand(COMMAND_MOTOR_2,_PWMperiodInt,_PWMperiodInt-abs(val));
      }
    }
  }else{
    if(motor==1){
